...
def filter_claude_comments(
    ...
    latest_codex_reply = latest_claude_reply_by_thread(comments)
    latest_issue_ack = latest_claude_issue_reply_time(comments)
    codex_comments = [c for c in comments if is_claude_bot_user(c.get("user", {}))]
...
def raise_on_human_feedback(
    ...
    codex_review_comments = filter_claude_review_issue_comments(issue_comments)
